...
// build. They are deliberately optional for legacy bundles.
...
type Record struct {
	...
	// remain above for provenance and compatibility with schema v1 records.
...
// NormalizeLegacy upgrades in-memory defaults without changing the source
// file. A missing schema_version identifies the original schema v1 format.
func (r *Record) NormalizeLegacy() {
	if r != nil && r.SchemaVersion == 0 {
		r.SchemaVersion = MinSchemaVersion
	}
}
