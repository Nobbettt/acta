...
func TestUploadRunRejectsUnsafeArtifactPath(t *testing.T) {
	...
	writeFile(t, filepath.Join(runDir, "acta-events.jsonl"), strings.Join([]string{
		...
		`{"schema_version":1,"run_id":"run-1","sequence":2,"timestamp":"2026-07-06T12:00:01Z","source":"acta","type":"run.completed","payload":{},"artifact_refs":[{"kind":"run_record","path":"../run.json"}]}`,
		"",
	}, "\n"))
...
func TestScanEventsRejectsInvalidReplayContract(t *testing.T) {
	started := `{"schema_version":1,"run_id":"run-1","sequence":1,"timestamp":"2026-07-06T12:00:00Z","source":"acta","type":"run.started","payload":{}}`
	completed := `{"schema_version":1,"run_id":"run-1","sequence":2,"timestamp":"2026-07-06T12:00:01Z","source":"acta","type":"run.completed","payload":{}}`
	cases := map[string][]string{
		"wrong schema":     {strings.Replace(started, `"schema_version":1`, `"schema_version":3`, 1), completed},
		...
		"event after terminal": {started, completed,
			`{"schema_version":1,"run_id":"run-1","sequence":3,"timestamp":"2026-07-06T12:00:02Z","source":"acta","type":"agent.message","payload":{}}`},
	}
