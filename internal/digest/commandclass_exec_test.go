...
func TestClassifyExecArchiveAndPermission(t *testing.T) {
	runExecCases(t, []execCase{
		...
		{name: "tar repeated relative destinations escape through a symlink", command: "tar -C build -C .. -xf release.tgz", want: []string{"workspace.escape"}},
		{name: "tar repeated relative destinations after archive escape through a symlink", command: "tar -xf release.tgz -C build -C ..", want: []string{"workspace.escape"}},
