...
// validTestRecord returns the smallest record that passes current-schema
// validation, for tests whose subject is raw-stream digestion rather than
// record validity.
func validTestRecord(dir, rawName string) runrecord.Record {
	exit := 0
	started := time.Date(2026, 7, 3, 12, 0, 0, 0, time.UTC)
	return runrecord.Record{
		SchemaVersion: runrecord.SchemaVersion,
		// The build producer, so live and re-digest producers agree in tests
		// that compare both paths.
		...
		RawStderrPath:      filepath.Join(dir, "agent.stderr.log"),
		RawStdoutArtifact:  rawName,
		RawStderrArtifact:  "agent.stderr.log",
		PromptSource:       "test",
...
func TestFromRunDirUsesRecordRawPath(t *testing.T) {
	...
	rec := validTestRecord(dir, rawName)
