...
func TestWriteRecordRejectsUnreadableSize(t *testing.T) {
	...
	err := WriteRecord(t.TempDir(), &runrecord.Record{SchemaVersion: 1, ID: "oversized", IssueBody: &issueBody})
...
func TestRunsDirExcludeTargetsOnlyCurrentRun(t *testing.T) {
	cwd := t.TempDir()
	runDir := filepath.Join(cwd, ".acta", "runs", "run-2")
	got := runsDirExclude(cwd, runDir)
	if len(got) != 1 || got[0] != ".acta/runs/run-2" {
		t.Fatalf("runsDirExclude() = %v", got)
	}
}
