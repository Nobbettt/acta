...
type Options struct {
	...
	// omit. Nothing such as all of .acta is excluded implicitly.
...
func Run(ctx context.Context, opts Options, stdout io.Writer, stderr io.Writer) (*runrecord.Record, error) {
	...
	defaultRunsDir := filepath.Join(cwd, ".acta", "runs")
	...
	producer := version.Current()
	record := &runrecord.Record{
		SchemaVersion: runrecord.SchemaVersion,
		Producer: runrecord.Producer{
			Name: "acta", Version: producer.Version, Commit: producer.Commit, Date: producer.Date,
		},
		ID:                      runID,
		TraceID:                 tr.TraceID(),
		Agent:                   adapter.Name(),
...
// runsDirExclude returns only this run's final bundle path relative to the
// workspace. The bundle does not exist until after final Git capture, but an
// exact exclusion keeps the invariant without hiding tracked sibling bundles
// or unrelated files under the same runs root.
func runsDirExclude(cwd, runDir string) []string {
	rel, err := filepath.Rel(cwd, runDir)
